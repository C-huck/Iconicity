...
def sem_scores(sent_sort):
    ...
    for sent in sent_sort:
        ...
        for x in word_list:
            ...
            for x in w2:
                if w1 in str(x): #similarity scores decrease if most frequent synset is used, so string match
...
sent_sort = read_data("action-label-data.csv")
sent_scores = sem_scores(sent_sort)
